...
class Learn:
    ...
    def test1(self):
        ...
        # 三个基学习器
        dt_clf = tree.DecisionTreeClassifier(random_state=0,max_leaf_nodes=2,min_samples_split=10)
        rf_clf = RandomForestClassifier()
        svm_clf = SVC(probability=True)
        log_clf = LogisticRegression()
        nbrs_clf = neighbors.KNeighborsClassifier()
        nb_clf = GaussianNB()
        GBC_clf = GradientBoostingClassifier(random_state=0,learning_rate=0.3,max_depth=1)
        clf = AdaBoostClassifier(
            VotingClassifier(estimators=[("dt_clf", dt_clf), ("rf_clf", rf_clf), ("log_clf", log_clf)], voting="soft"),
            algorithm="SAMME",
            n_estimators=200, learning_rate=0.9)
    ...
    def test2(self):
        ...
        # 投票分类器
        clf = AdaBoostClassifier(
            VotingClassifier(estimators=[("rf", dt_clf), ("svc", rf_clf), ("nc", svm_clf)], voting="soft"),
            algorithm="SAMME",
    ...
    def test3(self):
        ...
        clf = AdaBoostClassifier(
            ...
            algorithm="SAMME",
            n_estimators=50, learning_rate=0.6)
        vt = VotingClassifier(
            estimators=[("xgb_clf", xgb_clf), ("svc", log_clf), ("dt_clf", nb_clf)], voting="soft")
        return rf_clf
    ...
    def initial_model1(self):
        ...
        clfb = BaggingClassifier(base_estimator=vc)
        return  clfb
    ...
    def initial_model2(self):
        ...
        GBC_clf = GradientBoostingClassifier(random_state=0, learning_rate=0.3, max_depth=1)
        vc = VotingClassifier(estimators=[("rf", dt_clf), ("svc", GBC_clf), ("nc", svm_clf)], voting="soft")
        clfb = BaggingClassifier(base_estimator=vc)
        return clfb
    ...
    def initial_model3(self):
        ...
        clf = AdaBoostClassifier(vc,algorithm="SAMME",n_estimators=50, learning_rate=0.6)
        clfb = BaggingClassifier(base_estimator=vc)
        return clfb
